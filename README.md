...
| Parameter | Type | Description |
| --- | --- | --- |
| clientOrderId | string, >=8 characters, <= 30 characters, required | unique order id generated by client |
...
| Parameter | Type | Description |
| --- | --- | --- |
| clientOrderId | string, >=8 characters, <= 30 characters, required | the same an in your order |
| cancelRequestClientOrderId | string, >=8 characters, <= 30 characters, required | unqiue id generated by client |
| symbol | string, required | the same an in your order |
| side | `buy` or `sell`, required | the same an in your order |
| price | decimal, required | the same an in your order |
| quantity | int | the same an in your order |
| type | `limit` or `market` | the same an in your order |
| timeInForce | `GTC` - Good-Til-Canceled <br>`IOK` - Immediate-Or-Cancel<br>`FOK` - Fill-Or-Kill<br>`DAY` - day | the same an in your order |
...
post data: clientOrderId=11111112&cancelRequestClientOrderId=38257825798349578945&symbol=BTCUSD&side=buy&price=0.1&quantity=100&type=limit&timeInForce=GTC
```
